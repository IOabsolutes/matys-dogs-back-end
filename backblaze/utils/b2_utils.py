...
def initialize_b2api():
    required_env_varibles = ["APPLICATION_KEY_ID", "APPLICATION_KEY", "BUCKET_NAME_IMG"]
...
def upload_to_backblaze(byte_arr, file_obj):
    ...
    file_info = bucket.upload_bytes(byte_arr.getvalue(), file_name=webp_file_name)
...
def converter_to_webP(file_obj):
    try:
        ...
        raise ValidationError(
            detail={"description": f"Помилка перетвореня у webp: {e}"}
        )
...
def compress_image(
    ...
    step_quality=QUALITY_STEP,
    step_lossy_quality=LOSSY_QUALITY_STEP,
